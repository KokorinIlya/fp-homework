...
buildChurchOrLibrary city toBuild =
  case churchOrLibrary city of
    ...
    x -> Left ("Cannot build " ++ show toBuild ++ ", because " ++ show x ++ "is built")
...
addLord city lord =
  case castle city of
    NoCastle -> Left "Cannot add lord, because house is not built"
    OnlyCastle -> Right city {castle = CastleWithLord lord}
    CastleWithLord (LordName lordName) -> Left ("Lord " ++ lordName ++ " is already present")
    CastleWithWalls -> Right city {castle = CastleWithWallsAndLord lord}
    CastleWithWallsAndLord (LordName lordName) -> Left ("Lord " ++ lordName ++ " is already present")
